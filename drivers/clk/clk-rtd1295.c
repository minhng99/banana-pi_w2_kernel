...
#include <dt-bindings/clock/realtek,rtd129x.h>
#include <linux/clk.h>
...
static void __init rtd1295_clk_init(struct device_node *node)
{
	...
	clks[CC_PLL_SCPU] = rtd_scpu(base + 0x500, "pll_scpu", osc);
	clks[CC_PLL_BUS] = rtd_nf_ssc(base + 0x520, "pll_bus", osc);
	clks[CC_PLL_BUS_DIV2] = clk_register_fixed_factor(NULL, "pll_bus_div2", "pll_bus", 0, 1, 2);
	clks[CC_CLK_SYS] = clk_register_mux(NULL, "clk_sys", clk_sys_parents, 2, 0, base + 0x30, 0, 1, CLK_MUX_READ_ONLY, NULL);
	clks[CC_PLL_BUS_H] = rtd_nf_ssc(base + 0x540, "pll_bus_h", osc);
	clks[CC_CLK_SYSH] = clk_register_fixed_factor(NULL, "clk_sysh", "pll_bus_h", 0, 1, 1);
	clks[CC_PLL_DDSA] = rtd_nf_ssc(base + 0x560, "pll_ddsa", osc);
	clks[CC_PLL_DDSB] = rtd_nf_ssc(base + 0x580, "pll_ddsb", osc);
	clks[CC_PLL_VODMA] = rtd_mno_ctrl(base + 0x260, "pll_vodma", osc);
	clk_register_fixed_factor(NULL, "clk_vodma", "pll_vodma", 0, 1, 1);
	clks[CLK_EN_VO] = clk_register_gate(NULL, "clk_en_vo", "clk_vodma", CLK_IGNORE_UNUSED, base + 0xc, 15, 0, NULL);
	clks[CC_PLL_VE1] = rtd_mno_ctrl(base + 0x114, "pll_ve1", osc);
	clks[CC_PLL_VE2] = rtd_mno_ctrl(base + 0x1d0, "pll_ve2", osc);
	clk_register_mux(NULL, "clk_ve1", clk_ve_parents, 4, 0, base + 0x4c, 0, 2, CLK_MUX_READ_ONLY, NULL);
	clks[CLK_EN_VE1] = clk_register_gate(NULL, "clk_en_ve1", "clk_ve1", CLK_IGNORE_UNUSED, base + 0xc, 12, 0, NULL);
	clk_register_mux(NULL, "clk_ve2", clk_ve_parents, 4, 0, base + 0x4c, 2, 2, CLK_MUX_READ_ONLY, NULL);
	clks[CLK_EN_VE2] = clk_register_gate(NULL, "clk_en_ve2", "clk_ve2", CLK_IGNORE_UNUSED, base + 0xc, 13, 0, NULL);
	clk_register_mux(NULL, "clk_ve3", clk_ve_parents, 4, 0, base + 0x4c, 4, 2, CLK_MUX_READ_ONLY, NULL);
	clks[CLK_EN_VE3] = clk_register_gate(NULL, "clk_en_ve3", "clk_ve3", CLK_IGNORE_UNUSED, base + 0xc, 29, 0, NULL);
	clks[CC_PLL_GPU] = rtd_nf_ssc(base + 0x5a0, "pll_gpu", osc);
	clk_register_fixed_factor(NULL, "clk_gpu", "pll_gpu", 0, 1, 1);
	clks[CLK_EN_GPU] = clk_register_gate(NULL, "clk_en_gpu", "clk_gpu", CLK_IGNORE_UNUSED, base + 0xc, 11, 0, NULL);
	clks[CC_PLL_ACPU] = rtd_nf_ssc(base + 0x5c0, "pll_acpu", osc);

	for (i = 0; i < ARRAY_SIZE(rtd1295_gates1); i++) {
		if (!rtd1295_gates1[i])
			continue;
		clks[16 + i] = clk_register_gate(NULL, rtd1295_gates1[i], NULL, CLK_IGNORE_UNUSED, base + 0xc, i, 0, NULL);
	}

	for (i = 0; i < ARRAY_SIZE(rtd1295_gates2); i++) {
		if (!rtd1295_gates2[i])
			continue;
		clks[48 + i] = clk_register_gate(NULL, rtd1295_gates2[i], __clk_get_name(osc), CLK_IGNORE_UNUSED, base + 0x10, i, 0, NULL);
	}
